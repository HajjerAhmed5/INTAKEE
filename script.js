...
function setActiveTab(tabId) {
  ...
  if (['upload', 'settings'].includes(tabId)) { hide(searchBar); hide(loginBtn); }
  else { show(searchBar); show(loginBtn); }
...
function initTabs() {
  ...
  window.addEventListener('hashchange', () => setActiveTab((location.hash || '#home').slice(1)));
}

// ===== Settings accordion =====
function initSettingsAccordion() {
  $$('.settings-label').forEach(label => {
    label.addEventListener('click', () => label.closest('.settings-section')?.classList.toggle('open'));
  });
}
...
async function handleLogout() { try { await signOut(auth); alert('Logged out.'); } catch (e) { alert(e.message); } }
function promptLoginIfNeeded() { if (!auth.currentUser) showAuthModal(); }
function handleLogoutOrPrompt() { if (!auth.currentUser) return showAuthModal(); handleLogout(); }
...
// ===== Upload: type-aware labels + real Firebase upload =====
function refreshUploadForm() {
  ...
  switch (type) {
    case 'video':
      ...
      help.textContent = 'Upload a standard video (MP4/WebM). Thumbnail shows in feed.';
      break;
    case 'clip':
      fileLabel.textContent = 'Clip File';
      fileInput.accept = 'video/*';
      help.textContent = 'Upload a short vertical clip (MP4/WebM). Recommended under 60s.';
      break;
    case 'podcast-video':
      fileLabel.textContent = 'Podcast Video File';
      fileInput.accept = 'video/*';
      help.textContent = 'Upload a video podcast. Thumbnail shows only in the feed.';
      break;
    case 'podcast-audio':
      fileLabel.textContent = 'Podcast Audio File';
      fileInput.accept = 'audio/*';
      help.textContent = 'Upload an audio podcast (MP3/M4A/WAV). Thumbnail is a static cover.';
      break;
  }
}
...
async function handleUpload() {
  ...
  try {
    ...
    const mainRef = storageRef(st, mainPath);
    ...
    if (thumb) {
      ...
      const thumbRef = storageRef(st, thumbPath);
    ...
    const doc = await addDoc(collection(db, 'posts'), {
      uid, type, title, description: desc || '',
      fileUrl, thumbUrl,
...
async function loadFeeds() {
  // Trending = latest 12
  await fillGrid('#trending-feed', query(collection(db, 'posts'), orderBy('createdAt', 'desc'), limit(12)));
  // Videos
  await fillGrid('#video-feed',
    query(collection(db, 'posts'), where('type', 'in', ['video']), orderBy('createdAt', 'desc'), limit(12)));
  // Podcasts: both audio and video
  await fillGrid('#podcast-feed',
    query(collection(db, 'posts'), where('type', 'in', ['podcast-audio','podcast-video']), orderBy('createdAt','desc'), limit(12)));
  // Clips
  await fillGrid('#clips-feed',
    query(collection(db, 'posts'), where('type', 'in', ['clip']), orderBy('createdAt', 'desc'), limit(12)));
}
...
async function fillGrid(sel, q) {
  ...
  try {
    ...
    if (snap.empty) { el.textContent = el.id.includes('video') ? 'No videos yet' :
                                   el.id.includes('podcast') ? 'No podcasts yet' :
                                   el.id.includes('clips') ? 'No clips yet' : 'No trending posts'; return; }
    el.innerHTML = '';
    snap.forEach(doc => {
      const d = doc.data();
      const card = document.createElement('div');
      card.className = 'video-card';
      const cover = d.thumbUrl || (d.type === 'podcast-audio' ? 'default-audio-cover.png' : '');
